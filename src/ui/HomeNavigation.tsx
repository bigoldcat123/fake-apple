...
export default function HomeNavigation({data} : {data:{
    ...
    function hideBanner(size: any) {
        if (head.current && downMenu.current) {
            // head.current.style.transition = ''
            head.current.style.height = '2.5rem'
            timmer.push(setTimeout(() => {
                downMenu.current!.style.opacity = '0'
                // head.current!.style.transition = 'height 0.5s ease-in-out'
            }, 300))
        }
    }
    function hideBannerWithoutAnimation(number:any) {
        if (head.current && downMenu.current) {
            head.current.style.transition = ''
            head.current.style.height = '2.5rem'
            timmer.push(setTimeout(() => {
                downMenu.current!.style.opacity = '0'
                head.current!.style.transition = 'height 0.3s ease-in-out'
            }, 300))
        }
    }
    return (
        <>
            <div ref={head} className=" overflow-hidden scale-y-100 relative z-[3] backdrop-blur-md justify-center" style={{ height: '2.5rem', position: pathName == '/' ? 'fixed' : 'absolute', width: '100%', transition: 'height 0.3s ease-in-out' }}>
                <div className=" text-[0.72rem] font-[540] mx-auto  w-[1005.53px] " style={{ zIndex: '100', alignItems: 'center', display: 'flex', gap: '3rem', height: '2.5rem', justifyContent: 'center', position: 'relative' }}>
                    ...
                    <div className=" cursor-pointer w-auto" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(0)}><Link href={'/store'}>Store</Link></div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(1)}><Link href={'/mac'}>Mac</Link></div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(2)}>iPad</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(3)}>iPhone</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(4)}>Watch</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(5)}>Vision</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(6)}>AirPods</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(7)}>Home</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(8)}>Entertainment</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(9)}>Accessories</div>
                    <div className=" cursor-pointer" onClick={() => {hideBannerWithoutAnimation(0)}} onMouseEnter={(e) => showBanner(10)}>Support</div>
            ...
            <div className=" text-center text-sm bg-slate-200"> 一条 广告 哦</div>
        </>
    )
}